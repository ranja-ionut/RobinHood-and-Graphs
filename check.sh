...
test_p3() {
    ...
    run_problem p3 3 verif_double
}

# Task 4 - Bonus
test_p4() {
    n_tests=25
    src_names=(bonus.c bonus.cpp Bonus.java)
    tests=( $(seq 0 $(($n_tests - 1))) )
    points=( $(printf '1.0 %.0s' $(seq 0 $(($n_tests - 1)))) )
    pmax=25

    run_problem bonus 4 verif_bonus
}
...
check_coding_style() {
    ...
    cpp_sources=$(find -L . \( -name '*.cpp' ! -name '*verif*' \) -o -name '*.c' -o -name '*.h' -o -name '*.hpp')
