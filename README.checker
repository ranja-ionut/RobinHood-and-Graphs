...
Rulare: ./check.sh help
    Usage:
        ...
        ./check.sh task_id         # run only one problem (e.g. number)
        ./check.sh cs              # run only the coding style check

Mentiuni:
    - Makefile trebuie sa se afle in directorul curent.
    - Sursele trebuie sa se afle in directorul curent sau
      in subdirectoare ale acestuia (de ex src/).
    - Makefile-ul trebuie sa contina regulile specificate in enuntul temei
      (urmariti si exemplele Makefile_cpp.example si Makefile_java.example) .
